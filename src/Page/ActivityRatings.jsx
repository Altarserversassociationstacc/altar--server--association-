...
import { 
  ...
  FaTachometerAlt, FaCalendarDay,
  FaChevronLeft, FaChevronRight, FaChurch, FaUsers, FaTshirt
} from 'react-icons/fa';
...
const ActivityRatings = ({ 
  ...
  console.log("RAW DATA FROM DATABASE:", allMeetings);
  ...
  const liveLogs = (allMeetings || []).map(meeting => {
    ...
    // We use EXACT matching to safely fix old database entries.
    // This prevents a title like "Meeting about practice" from being wrongly tagged.
    if (meeting.title) {
  ...
  // 🚀 THIS DECIDES WHAT TO SHOW AT THE BOTTOM
  const displayLogs = selectedDateString 
    ? activeLogs 
    : [...filteredLogs].sort((a, b) => new Date(b.date) - new Date(a.date)).slice(0, 10);

  // Helper function to render activity icons dynamically
  const renderActivityIcon = (type) => {
    switch (type) {
      case 'Practice': return <FaUsers size={8} />;
      case 'Cloth Washing': return <FaTshirt size={8} />;
      case 'Meeting': default: return <FaChurch size={8} />;
    }
  };

  const getActivityBadgeStyle = (type) => {
    switch (type) {
      case 'Practice': return 'bg-purple-950 text-purple-400 border-purple-900/30';
      case 'Cloth Washing': return 'bg-amber-950 text-amber-400 border-amber-900/30';
      case 'Meeting': default: return 'bg-blue-950 text-blue-400 border-blue-900/30';
    }
  };
    ...
    <div className="animate-fadeIn font-sans text-white pb-12 w-full space-y-8">
      ...
      <div className="flex flex-col lg:flex-row justify-between gap-6 bg-black/40 backdrop-blur-xl border border-white/10 p-6 rounded-3xl shadow-2xl">
        ...
        <div className="flex-1 flex items-center gap-6 border-r-0 lg:border-r border-white/10 pr-0 lg:pr-6">
          ...
          <div>
            <h2 className="text-2xl font-black text-white tracking-tight uppercase">Attendance</h2>
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-center justify-end w-full lg:w-auto">
          <div className="w-full sm:w-auto">
            <label className="text-[8px] text-gray-500 uppercase tracking-widest block mb-1 pl-1">Activity Type</label>
            <select value={selectedType} onChange={(e) => { setSelectedType(e.target.value); setSelectedDateString(null); }} className="w-full bg-[#111] border border-white/20 text-white text-xs font-bold uppercase tracking-wider rounded-xl px-4 py-3 outline-none focus:border-[#d2b48c] cursor-pointer">
              <option value="All">All Activities</option>
              <option value="Meeting">Meeting</option>
              <option value="Practice">Practice</option>
              <option value="Cloth Washing">Cloth Washing</option>
            </select>
          </div>
          <div className="w-full sm:w-auto">
            <label className="text-[8px] text-gray-500 uppercase tracking-widest block mb-1 pl-1">Semester</label>
            <select value={selectedSemester} onChange={(e) => { setSelectedSemester(e.target.value); setSelectedDateString(null); }} className="w-full bg-[#111] border border-white/20 text-white text-xs font-bold uppercase tracking-wider rounded-xl px-4 py-3 outline-none focus:border-[#d2b48c] cursor-pointer">
          ...
          <div className="w-full sm:w-auto">
            ...
            <select value={selectedLevel} onChange={(e) => { setSelectedLevel(e.target.value); setSelectedDateString(null); }} className="w-full bg-[#111] border border-white/20 text-white text-xs font-bold uppercase tracking-wider rounded-xl px-4 py-3 outline-none focus:border-[#d2b48c] cursor-pointer">
      ...
      <div className="bg-[#111111] border border-white/10 rounded-3xl p-6 shadow-2xl max-w-3xl mx-auto">
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6">
          <div className="flex items-center gap-2">
            ...
            <select value={currentMonth} onChange={(e) => { setCurrentMonth(parseInt(e.target.value)); setSelectedDateString(null); }} className="bg-white/5 border-0 rounded-xl py-2 px-3 text-sm font-black uppercase tracking-wider text-[#d2b48c] cursor-pointer outline-none">
              {MONTHS.map((m, index) => <option className="bg-black text-white" key={m} value={index}>{m}</option>)}
            </select>
            <select value={currentYear} onChange={(e) => { setCurrentYear(parseInt(e.target.value)); setSelectedDateString(null); }} className="bg-white/5 border-0 rounded-xl py-2 px-4 text-sm font-black tracking-wider text-white cursor-pointer outline-none">
        ...
        <div className="flex items-center gap-4 border-b border-white/5 pb-4">
          ...
          <div>
            <h3 className="text-xl font-serif text-white uppercase tracking-wide">
              {selectedDateString ? `Activity Logs: ${selectedDateString}` : 'Recent Search'}
            </h3>
            <p className="text-xs text-gray-500 uppercase font-bold tracking-widest">
              {selectedDateString ? 'Displaying user participation on this date' : 'Showing the latest logs based on your filters'}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {displayLogs.length > 0 ? (
            displayLogs.map((session, index) => (
              <div key={index} className={`border p-5 rounded-2xl flex flex-col justify-between transition-all bg-gradient-to-b ${
                ...
                <div className="flex items-start justify-between">
                  <div>
                    <span className={`text-[8px] font-black uppercase tracking-wider px-2 py-0.5 rounded flex items-center gap-1 w-max border ${getActivityBadgeStyle(session.type)}`}>
                    ...
                    <span className="text-[10px] text-gray-500 font-medium block mt-1">{session.displayDate}</span>
                  </div>
                  {session.attended ? (
            ...
            <div className="col-span-1 md:col-span-2 text-center py-6">
              <p className="text-gray-500 font-medium tracking-wide">No activities found matching your filters.</p>
            </div>
          )}
        </div>
      </div>
